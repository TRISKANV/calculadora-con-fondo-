        ...
        try {
            if (archivo.name.lowercase().endsWith(".enc")) {
                ...
                val opciones = BitmapFactory.Options().apply {
                    inSampleSize = 4 // Optimización de RAM
                }
                val bitmap = BitmapFactory.decodeByteArray(bytesDescifrados, 0, bytesDescifrados.size, opciones)
                holder.ivThumbnail.setImageBitmap(bitmap)
                inputStream.close()
            } else {
                // CASO 2: ARCHIVO DESCARGADO (JPG, PNG, WEBP)
                // Cargamos la miniatura directamente del archivo
                val opciones = BitmapFactory.Options().apply {
                    inSampleSize = 4
                }
                val bitmap = BitmapFactory.decodeFile(archivo.absolutePath, opciones)
                holder.ivThumbnail.setImageBitmap(bitmap)
            }
        } catch (e: Exception) {
